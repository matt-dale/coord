...
class Coordination(object):
    ...
    def test_one_freq(self, test_freq, test=False, abs=abs):
        ...
        if len(coordinated_freqs) == 0:
            ...
            imd_triples = self.imd_calc.calculate_triple_beats_for_a_set_of_freqs(test_frequency_list)
        else:
            ...
            test_frequency_list.append_(test_freq)
            f = FrequencyList([])
            imd_thirds, imd_fifths = self.imd_calc.calculate_imd_between_one_set_of_freqs(f, explicit_test_frequency_list)
            imd_triples = self.imd_calc.calculate_triple_beats_for_a_set_of_freqs(test_frequency_list)
...
class IntermodCalculator(object):
    ...
    def calculate_triple_beats_for_a_set_of_freqs(self, freqs1, explicit_list=None):
        imd_triples = set()
    ...
    def calculate_triple_beats(self, f1, f2, f3):
        ...
        if self.triple_beats:
            ...
            b = f1 + f3 - f2
            c = f2 + f3 - f1
            ''' all possible combos for 3 xmitters
            # third order
            e = (2*f1) - f2
            f = (2*f2) - f1
            g = (2*f1) - f3
