...
numpy array - freqs ordered, cooefficient columns
set of good freqs to start
http://numba.pydata.org/

pandas dataframe, each row of each set of known good
sqlite if pandas is too slow

multiprocessing - pool of cpus -1
function to each worker - 
"""
...
class Coordination(object):
    ...
    def create_potential_freqs(self):
        ...
        while loop_freq < self.end_freq:
            if loop_freq not in self.coordinated_freqs.get_freq_values():
    ...
    def test_one_freq(self, test_freq):
        ...
        if test_freq in imd_triples:
            ...
            the below checks should be done at once, with numpy array math
            results set rounding np.around(a, decimals = 1) 
            array broadcasting will allow us to do math with test_freq and the lists

            thirds = np.array(combined_thirds_list)
            results = thirds - test_freq  will broadcast to give an array 
            """
            np_thirds = np.asarray(combined_thirds_list)
            np_fifths = np.asarray(combined_fifths_list)
            np_trips =  np.asarray(combined_triples_list)
            third_differences =  np_thirds - test_freq
            fifth_differences = np_fifths - test_freq
            triple_differences = np_trips - test_freq
            results3 = abs(third_differences) <= self.avoid_imd_thirds_by
            results5 = abs(fifth_differences) <= self.avoid_imd_fifths_by
            results_triple = abs(triple_differences) <= self.avoid_imd_triples_by
            ...
            for f in coordinated_freqs:
                f_difference = abs(np_thirds-f)
                if (f_difference <= self.avoid_imd_thirds_by).any():
                    self.uncoordinated_freqs.append_(test_freq)
                    return False
                f_difference = abs(np_fifths-f)
                if (f_difference <= self.avoid_imd_thirds_by).any():
                    self.uncoordinated_freqs.append_(test_freq)
                    return False
                if self.triple_beats:
                    f_difference = abs(np_trips-f)
                    if (f_difference <= self.avoid_imd_thirds_by).any():
                        self.uncoordinated_freqs.append_(test_freq)
                        return False
...
# test stuff
freq_array = np.dtype([('f1','f'), ('f2', 'f'), ('f3', 'f'),('2f1','f'), ('2f2', 'f'), ('2f3', 'f'),('3f1','f'), ('3f2', 'f'), ('3f3', 'f')]) 
# this array will have all the freqs in it for three xmitter calculations
coord = Coordination()
